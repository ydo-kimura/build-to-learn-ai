...
The dev server binds to `0.0.0.0:5173` (configured in `package.json` `docs:dev` script with `--host 0.0.0.0`).
...
The repository does not currently have a dedicated test CI workflow. Build verification is performed manually via `pnpm docs:build` during the AIDLC Build and Test stage. The `verify_curriculum_diagrams.py` script is run locally for asset integrity checks.
...
# Verify diagram integrity (checks SVG existence, alt text, bridge sentences)
python scripts/verify_curriculum_diagrams.py
```
