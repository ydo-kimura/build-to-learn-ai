...
The central config file is [`.vitepress/config.js`](../../.vitepress/config.js). It wraps the VitePress config with `withMermaid()` from `vitepress-plugin-mermaid` to enable inline Mermaid diagram rendering.
...
```javascript
srcExclude: ['**/aidlc-docs/**', '**/node_modules/**', '**/.agent/**', 'README.md']
```
...
The custom theme extends VitePress's DefaultTheme with one CSS file: [`.vitepress/theme/custom.css`](../../.vitepress/theme/custom.css).
...
```
Markdown (curriculum/*.md, en/curriculum/*.md)
    ↓
VitePress build (pnpm docs:build)
    ↓
Static HTML/CSS/JS in .vitepress/dist/
```
...
|---|---|---|
| `docs:dev` | `vitepress dev --host 0.0.0.0` | Dev server with hot reload |
| `docs:build` | `vitepress build` | Production build |
| `docs:preview` | `vitepress preview` | Preview built site |
...
Static assets are served from the `public/` directory. The repository maintains two parallel asset trees:

- `assets/units/<slug>/images/` — canonical source (where generation scripts write)
- `public/assets/units/<slug>/images/` — deployed copy (served by VitePress)

The [visual asset pipeline](../workflows/visual-pipeline.md) writes to `assets/` and the `public/assets/` copy serves as the runtime source. Both trees must stay in sync — the [verification script](../workflows/visual-pipeline.md#verification-scripts) checks this.
