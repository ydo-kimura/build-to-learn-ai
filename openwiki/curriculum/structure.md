...
- **Assets**: Separate `assets/` and `en/assets/` directories with per-unit image folders containing the same filenames

### Language Switcher

The VitePress locale config provides a language switcher in the nav bar that toggles between `/` and `/en/` while preserving the current page path. This is configured in [`.vitepress/config.js`](../../.vitepress/config.js) under the `locales` key.
