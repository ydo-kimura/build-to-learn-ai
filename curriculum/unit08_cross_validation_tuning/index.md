...
<img src="../../assets/units/unit08_cross_validation_tuning/images/diagram-concept.svg" alt="図解：5分割交差検証の仕組み" class="unit-diagram" />
...
| 手法の名前 | 仕組みの例え |
| :--- | :--- |
| **グリッドサーチ (Grid Search)** | 「テレビの色合い設定」で、明るさ（1〜10）とコントラスト（1〜10）の **全100通りの組み合わせを全て試して** 一番綺麗に見えるものを探す。確実だが時間がかかる。 |
| **ランダムサーチ (Random Search)** | 全通り試すのは時間がかかりすぎるので、 **ランダムに数十個の組み合わせをピックアップして** 試す。速くてそこそこ良い結果が出る。 |


下図は、ハイパーパラメータの組み合わせを試し、 **最高スコアの設定** を選ぶグリッドサーチです（図は max_depth × 学習率を探索する例です）。

<img src="../../assets/units/unit08_cross_validation_tuning/images/diagram-workflow.svg" alt="図解：ハイパーパラメータのグリッド探索" class="unit-diagram" />
...
今回は「乳がんの診断データ」と「ランダムフォレスト」を使って、 **「交差検証（5回テスト）」をしながら「全通りの設定を試す（グリッドサーチ）」** という、代表的なモデル選択手法（`GridSearchCV`）を実装してみましょう。
...
**【コードの解説】**
`.fit()` が終わると、`best_params_` に検証スコアが最も良かった設定が入り、`best_estimator_` にその設定で再学習されたモデルが入ります。最後に、モデル選択に使っていないテストデータで評価し、結果と限界を記録します。

なお、試したい設定の組み合わせが何百・何千通りにもなる場合、全通りを試すグリッドサーチでは時間がかかりすぎます。探索空間が広いときは、ランダムに組み合わせを選んで試す `RandomizedSearchCV`（ランダムサーチ）の方が効率的です。

```python
# (参考) RandomizedSearchCV の最小例
from sklearn.model_selection import RandomizedSearchCV

random_search = RandomizedSearchCV(
    estimator=RandomForestClassifier(random_state=42),
    param_distributions=param_grid,  # 探索範囲は同じ辞書でOK
    n_iter=5,       # 全通りではなく「5通りだけ」ランダムに試す
    cv=5,
    random_state=42
)
