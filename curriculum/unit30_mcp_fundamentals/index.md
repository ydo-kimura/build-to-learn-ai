...
<img src="../../assets/units/unit30_mcp_fundamentals/images/diagram-concept.svg" alt="図解：MCP architecture" class="unit-diagram" />
...
この問題を解決するため、Anthropic が提唱したオープンな共通標準規格が **Model Context Protocol (MCP)** です。MCP は、LLM（AIクライアント）と外部のデータ・ツールの連携インターフェースを標準化し、一度作ったツールを任意のMCP対応クライアントで即座に再利用できるようにする「AI界のUSB規格」です。
...
#### テキストによるシステム構成の代替表現
1. **Client（クライアント）**: ユーザーが操作するUI環境（Cursor IDE, Claude Desktop アプリなど）であり、LLMとの対話やツールの承認を担当します。
2. **Host（ホスト）**: エージェントを駆動する仲介サーバー・スクリプト。クライアントからの意図を受け取り、適切な MCP サーバーに指示を中継します。
3. **Server（サーバー）**: 実際のツールやデータをホストするプロセス。自律的に動き回るのではなく、Host からの要求（JSON-RPC経由）に対して、特定のデータ（Resources）やアクション（Tools）を厳密に提供します。
...
* **Resources (リソース)**:
  ...
  * URL形式（例: `mysql://customer_db/profile`）でアドレッシングされ、エージェントは自律的にこのデータを取得して文脈理解に活用します。
* **Prompts (プロンプト)**:
...
<img src="../../assets/units/unit30_mcp_fundamentals/images/diagram-workflow.svg" alt="図解：Build a server" class="unit-diagram" />
...
4. Pythonスクリプトとして完全に実行可能（stdioで起動する）な形でコードを完成させてください。
...
以下は、サポート用 MCP サーバーを設計・実装した完全なコード例です。
