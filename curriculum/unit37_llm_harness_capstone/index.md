...
第4章（Unit 22〜34）において、LLMのAPI利用、ゼロからのRAG（検索拡張生成）構築、LangChainの基礎、プロンプトの連結（Chaining）、 Web UIチャットボット、および自律動作する AIエージェントの構築までを網羅的に学習してきました。

この最初の総合演習ユニット（Unit 37）では、これらすべての集大成として、エンタープライズなAIシステム開発で重要な **「ハーネスエンジニアリング（Harness Engineering / LLM評価ハーネス）」** を学び、構築します。
...
なお、本ユニットの「評価（Evaluation）」と、Unit 40 で学ぶ「ガードレール（Guardrails / 防御）」は役割が異なります。本ユニットの評価ハーネスは、プロンプトやRAGの変更が品質を改善したか（デグレードしていないか）を **リリース前・改善時にオフラインで測定する品質測定器** です。一方、Unit 40 のガードレールは、本番稼働中のシステムで入出力をリアルタイムに検問し、有害な入力や危険な出力をその場で遮断する **稼働中のランタイム防御機構** です。両者を組み合わせることで、開発時の測定と運用時の防御を分担できます。


下図は、 **Generation → Evaluation → Feedback** の二重ハーネス構成です。

<img src="../../assets/units/unit37_llm_harness_capstone/images/diagram-concept.svg" alt="図解：二重ハーネスの構成" class="unit-diagram" />

---


下図は、本番運用時にこの評価器を組み込む発展形で、ポリシー検査・LLM ジャッジ・フォールバックを重ねる **多層防御** の構成です（ランタイム防御の詳細は Unit 40 で扱います）。

<img src="../../assets/units/unit37_llm_harness_capstone/images/diagram-workflow.svg" alt="図解：多層防御の仕組み" class="unit-diagram" />
...
| **判定のブレ** | 1つのプロンプトで判断すると、「丁寧だが的外れ」な回答に高得点を与える可能性がある。 | 評価次元（Relevance と Tone）を分けることで、原因を分析しやすくなる。ただしLLMの採点揺らぎは残る。 | 複数回評価、人手評価との一致率、固定テストセットでの再現性を測定する。 |
...
### アプローチA（言葉遣い特化・単一評価ハーネス）の実装コード

まずは比較のベースラインとして、「言葉遣い・ホスピタリティ」だけを単一次元で採点するアプローチAを実装します。
...
# アプローチA: Tone & Quality のみに焦点を当てた単一評価プロンプト
EVAL_TONE_PROMPT = """あなたは高級ホテルの接客品質監査役です。
提供された「AIの回答」の言葉遣い・ホスピタリティ・礼儀正しさ (Tone & Quality) のみを、以下の基準で厳密に採点してください。

【評価基準】
5: 最高級ホテルのスタッフとして完璧な敬語と、温かい歓迎の言葉（「お気をつけてお越しくださいませ」等）が含まれる。
4: 丁寧な敬語だが、歓迎・気遣いの一言が不足している。
3: 失礼ではないが、「大丈夫です」「〜ですね」などカジュアルすぎる表現が含まれる。
2: 敬語が崩れており、接客としては不適切なフランクさがある。
...
def run_tone_evaluation_harness(case):
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
...
print("=== アプローチA: 言葉遣い特化ハーネスの実行 ===")
for case in relevance_test_cases:
    result_a = run_tone_evaluation_harness(case)
    print(f"[{case['id']}] Tone: {result_a['tone_score']} / 5")
    print(f"  採点理由: {result_a['tone_reason']}")
```

アプローチAを実行すると、`case_C_ideal` は 5 点、`case_D_bad_tone` は 1 点となり、言葉遣いの乱れは確実に検知できます。しかし、「敬語は完璧だが質問に答えていない回答」が来た場合でも高得点を与えてしまう、という限界がこの単一次元評価には残ります。この弱点を補うのが、次のアプローチBです。

### アプローチB（2次元多角的評価ハーネス）の完全実装コード
...
* **本番運用での最終適用決定** :
  ...
  * **意思決定の根拠** :
    1. 単一の「言葉遣い」だけの評価（アプローチA）では、「丁寧だが的確な情報を言っていない（もっともらしい嘘＝ハルシネーションを含む）」回答に対して高得点を出してしまう脆弱性があります。
    2. アプローチBでは、`relevance_score` と `tone_score` を分けたGatekeeperルールを構築できます。閾値は業務要件と人手評価で決め、評価の誤りや未検知を前提に監視・再評価します。
    3. 採点理由が「的確さ」と「口調」で別々に出力されるため、AIエンジニアがどの観点を修正すべきか分析しやすくなります。生産性や判定の安定性は、変更前後のテスト結果で確認します。
