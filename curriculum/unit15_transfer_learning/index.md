...
転移学習では、大規模なデータセットで学習済みのモデルをダウンロードして使います。モデルは「丸い形」「エッジ」などの汎用的な特徴を利用できる場合がありますが、対象データとの違い、ラベル品質、前処理、過学習を検証する必要があります。
...
式で書くと、ResNet のブロックは入力 `x` をショートカット経路でそのまま運び、層の変換結果に足し合わせます（ `output = F(x) + x` ）。層の学習がうまくいかなくても入力の情報がそのまま出力に届くため、深いネットワークでも情報が迷子になりにくいのです。


下図は、 **凍結したバックボーン** と、自分のデータで学習する **新しい出力層** の構成です。

<img src="../../assets/units/unit15_transfer_learning/images/diagram-concept.svg" alt="図解：転移学習の考え方（凍結したバックボーン＋新しい出力層）" class="unit-diagram" />
...
<img src="../../assets/units/unit15_transfer_learning/images/diagram-workflow.svg" alt="図解：ResNet18 を使った転移学習の流れ" class="unit-diagram" />

## 2. 実装例 (Implementation Example)

ここでは、PyTorchを使って「ResNet18」という一流シェフを雇い、私たちがやりたい「犬と猫の2値分類（2クラス分類）」専用にカスタマイズ（転移学習）するコードを書いてみましょう。
なお、事前学習済みモデルは「ImageNet の統計（mean/std）で正規化した 224×224 サイズの入力」を前提としている点に注意してください（実データを使う場合のリサイズと正規化は Unit 16 で実践します）。
...
「凍結（`requires_grad = False`）」とは、 **既存の重みを学習中に更新しない** という意味です。事前学習で得た特徴抽出の知識をそのまま保ちつつ、新しく付け替えた層だけを学習させます。
また、MobileNet V2 の `classifier` は「Dropout → Linear」という `nn.Sequential` になっているため、置き換えの対象は最後の Linear 層である `classifier[1]` になります。`classifier` 全体を置き換えるのではなく、Linear 層だけを新しいものにすり替えるのがポイントです。
