...
事前に `pip install langchain-openai langchain-core` を実行し、環境変数に `OPENAI_API_KEY` を設定してください。現行のLangChainでは、インメモリ履歴として `InMemoryChatMessageHistory` を `langchain_core.chat_history` から読み込みます。
