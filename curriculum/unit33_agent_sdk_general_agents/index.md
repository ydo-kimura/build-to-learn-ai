# Unit 32: Agent SDK: 汎用・業務自動化

<p class="unit-hero">
  <img src="../../assets/units/unit32_agent_sdk_general_agents/images/hero.png" alt="ヒーロー画像：Agent SDK: General Agents" />
</p>

Unit 29〜31 では、スクラッチでの ReAct 実装や MCP、smolagents といった **OSSベースのアプローチ** を用いて、自律型エージェントの基礎とツール統合を学びました。これらは柔軟性が高くプロトタイピングに優れていますが、エンタープライズ環境では、スケーラビリティ・セキュリティ・状態管理を自前で設計する負担が課題となります。本ユニットでは、こうした課題を解決するために設計された **商用 Agent SDK** によるエンタープライズ向けのエージェント構築を学びます。

商用のAIシステム構築において、OSSのフレームワーク（LangGraphやsmolagentsなど）と並び、主要なクラウド/AIベンダーが提供するマネージドな **商用 Agent SDK** の重要性が高まっています。
本ユニットでは、エンタープライズの要件を満たすために設計された商用 Agent SDK のアーキテクチャ、永続状態管理、およびビジネス機能とのシームレスな統合手法について学びます。

---

## 1. 説明フェーズ (Explanation)

<img src="../../assets/units/unit32_agent_sdk_general_agents/images/diagram-concept.svg" alt="図解：General agent" class="unit-diagram" />
...
これに対し、AIベンダーが提供するマネージド Agent SDK は、以下のようなエンタープライズ向けの強力な機能が最初から統合されています。

1. **インフラのフルマネージド化**: サーバーレスでのエージェントホスティング、自動スケーリング。
2. **堅牢な状態永続化 (Episodic Memory)**: 会話セッションやコンテキストを自動でセキュアなマネージドデータベースに保存・復元。
3. **安全機能のビルトイン**: ガードレール（不適切表現や機密情報の検知・遮断）の標準統合。

### 1.2 主要ベンダーの Agent SDK アーキテクチャ

#### A. OpenAI Agents SDK
OpenAIの **Assistants API / Agents SDK** は、インプロセス（アプリケーション側）の状態管理からリモートクラウド（OpenAI側）への移行を先導した代表例です。
- **エージェント間委譲 (Handoffs)**: あるエージェントから別の特化型エージェントへ、会話のコンテキストを維持したまま制御を動的に引き渡す機能。
- **実行状態の永続化 (Snapshotting / Thread Management)**: 会話履歴（Thread）が自動的にOpenAIのサーバー側に保存され、開発者はスレッドIDを指定するだけで、以前の状態からシームレスに会話を再開できます。

#### B. AWS Bedrock AgentCore SDK & Strands Agents
AWSの **Bedrock AgentCore SDK** (`bedrock-agentcore-sdk-python`) は、AWSのエンタープライズ向けインフラと深く統合されたエージェント開発基盤です。
...
- **分散型エージェント（Llama Agents）**: ローカル環境からマルチクラウドまで、同一の標準APIを用いてエージェントをシームレスに移植可能にします。

### 1.3 ビジネス連携機能と決済（Payments）の統合
業務自動化エージェントは、情報の検索や要約に留まらず、**「自律的なアクションの完遂」**を期待されます。
これには、**Episodic Memory（長期文脈記憶）** を用いて「ユーザーが過去にどの商品を好んだか」を記憶しつつ、**Stripe や Coinbase などの外部決済API**（あるいはベンダーが模索する `AgentCore Payments` などのエージェント向け自律型ウォレット決済）と連携し、予約や支払いを自動で安全に実行するアーキテクチャが含まれます。
...
<img src="../../assets/units/unit32_agent_sdk_general_agents/images/diagram-workflow.svg" alt="図解：Use cases" class="unit-diagram" />

## 2. 実践 (Practice)

ここでは、OpenAI Agents や AWS Bedrock AgentCore の本質的なアーキテクチャである **「Episodic Memory（永続セッション）と Handoffs（役割委譲）」** を理解するため、Python によるシミュレーション実装を行います。
...
if __name__ == "__main__":
    ...
    # ターン 2: 在庫検索 (Handoffが内部で起き、在庫エージェントが処理してReceptionに戻る)
    print("\n=== TURN 2 ===")
    res2 = orchestrator.handle_request(my_session, "laptop の在庫を検索してほしい")
    print(f"Agent -> {res2}")
    
    # ターン 3: 購入の要望 (Paymentエージェントへ遷移して決済)
    print("\n=== TURN 3 ===")
...
以下は、`DiscountAgent` を追加し、リピーター判定とカート価格の更新ロジックを統合した完全な Python 実装です。
...
# 拡張された ReceptionAgent の process メソッド (割引要求の検知追加)
# (実運用時は ReceptionAgent 内の条件分岐を以下のように拡張します)
"""
...
if __name__ == "__main__":
    ...
    # 状態のセットアップ (カートに laptop $1200 を追加済み、かつ過去の購入履歴に smartphone あり)
    session = store.get_session(session_id)
    session["user_name"] = "ボブ"
    session["cart"].append({"item": "laptop", "price": 1200})
    session["purchase_history"].append("smartphone")  # リピーター化
