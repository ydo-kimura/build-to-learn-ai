...
> 📚 **本章の用語** : **トークン化／分かち書き** はテキストを単位に分割する処理、**形態素解析** は日本語の文を形態素に分けて品詞や原形も調べる処理、**分散表現（Embedding）／埋め込み** は単語を密なベクトルに変換する表現です。本章では、文脈に依存しない単語ベクトルを指す場合は「分散表現（Embedding）」と表記します。
...
lemmas = [token.base_form for token in t.tokenize("私は今日図書館に行きました")]
print(tokens)  # 表面形: ['私', 'は', '今日', '図書館', 'に', '行き', 'まし', 'た']
print(lemmas)  # 原形:     ['私', 'は', '今日', '図書館', 'に', '行く', 'ます', 'た']
```
...
なお、3冊すべてに登場する一般的な単語「の」を各文書に含めたとします。このとき、`IDF(の) = log(3 ÷ 3) = 0` なので、文書内で何度出現しても「の」の TF-IDF は 0 です。TF が高いだけでは重要とは限らず、文書全体で珍しいかどうかも重要だと分かります。
...
> 💡 本Aでは「AI」の方が「データ」よりも高いスコアを獲得します。ただし、すべての文書に出る「の」のような単語は、どれだけ頻出でも IDF が 0 になるため重要語とはみなされません。TF-IDF は「文書内でよく出る」ことと「コーパス全体では珍しい」ことを組み合わせます。
