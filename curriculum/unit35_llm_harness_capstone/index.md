...
なお、本ユニットの「評価（Evaluation）」と、Unit 38 で学ぶ「ガードレール（Guardrails / 防御）」は役割が異なります。本ユニットの評価ハーネスは、プロンプトやRAGの変更が品質を改善したか（デグレードしていないか）を **リリース前・改善時にオフラインで測定する品質測定器** です。一方、Unit 38 のガードレールは、本番稼働中のシステムで入出力をリアルタイムに検問し、有害な入力や危険な出力をその場で遮断する **稼働中のランタイム防御機構** です。両者を組み合わせること（開発時に評価で品質を担保し、運用時にガードレールで防御する）で、エンタープライズ水準の信頼性が実現します。


下図は、 **Generation → Evaluation → Feedback** の二重ハーネス構成です。

<img src="../../assets/units/unit35_llm_harness_capstone/images/diagram-concept.svg" alt="図解：二重ハーネスの構成" class="unit-diagram" />

---


下図は、本番運用時にこの評価器を組み込む発展形で、ポリシー検査・LLM ジャッジ・フォールバックを重ねる **多層防御** の構成です（ランタイム防御の詳細は Unit 38 で扱います）。

<img src="../../assets/units/unit35_llm_harness_capstone/images/diagram-workflow.svg" alt="図解：多層防御の仕組み" class="unit-diagram" />
