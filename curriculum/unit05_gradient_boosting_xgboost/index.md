...
<img src="../../assets/units/unit05_gradient_boosting_xgboost/images/diagram-concept.svg" alt="図解：Sequential boosting" class="unit-diagram" />
...
| **ブースティング** | 直列（前の失敗を次に活かす） | **圧倒的に精度が高い。** ただし計算に時間がかかり、調整（チューニング）が難しい。 |

### XGBoostとは？ 〜「最強・最速のブースティング」〜
ブースティングは精度が最強クラスですが、「直列（順番）に計算するからめちゃくちゃ時間がかかる」という弱点がありました。

そこで登場したのが**XGBoost (eXtreme Gradient Boosting)** です！
XGBoostは、本来直列でしかできない計算をアルゴリズムの工夫で無理やり「並列処理」できるようにし、**爆速化**を実現しました。さらに「データの欠損（空欄）を自動で処理する」などの便利な機能も詰め込まれています。
...
<img src="../../assets/units/unit05_gradient_boosting_xgboost/images/diagram-workflow.svg" alt="図解：XGBoost speed" class="unit-diagram" />
...
**【解答コードの解説】**
たった数十行のコードですが、これだけで世界最高峰の精度を誇るアルゴリズムを動かすことができました！XGBoostは表形式のデータ（エクセルで管理できるようなデータ）において、今でも最強のアルゴリズムとして実務で大活躍しています。
</details>
