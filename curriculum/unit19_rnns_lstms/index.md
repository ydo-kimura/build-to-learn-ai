...
**これまで扱った TF-IDF・Word2Vec の読み方：**
これらには、前の単語を時間方向に保持する仕組みがありません。
...
LSTMが「記憶の取捨選択」を実現できる秘密は、内部に **3つのゲート（門）** を持っていることにあります。厳密には、3つのゲートに加えて新しい記憶の候補（candidate cell state）を作る処理があり、この4つの要素でセル状態を更新します。
...
ここでは、PyTorchを使って「1文字入力されたら、次の1文字を予測する」というシンプルなRNN/LSTMモデルを作ってみましょう。"hello" という単語を学習させます。未インストールの場合は、先に `pip install torch` を実行してください。
...
**解答の解説：**
この例では同じ「p」でも系列内の位置が異なり、順番を追跡する必要があります。LSTMの隠れ状態が時系列の情報を保持する役割を担うため、入力系列から次の文字を予測できます。ただし、この極小データだけでLSTMの長期記憶能力全般を評価できるわけではありません。
