...
<img src="../../assets/units/unit27_prompt_chaining/images/diagram-concept.svg" alt="図解：Chain of prompts" class="unit-diagram" />
...
### Unit 25 の応用：LCEL によるチェーンの数珠つなぎ (Chaining)
Unit 25において、単一のチェーンを `prompt | model | parser` のようにパイプ記号 `|` で構築する方法を学習しました。

プロンプト Chaining の真髄は、**「すでに構築した複数の独立したチェーンを、さらに `|` で結合し、データのバケツリレーを行う」**点にあります。
...
<img src="../../assets/units/unit27_prompt_chaining/images/diagram-workflow.svg" alt="図解：Why chain" class="unit-diagram" />
...
3. **チェーン2**：`prompt2` で {catchphrase} を受け取り、ツイート文を生成する流れを作ります。
4. **チェーンの結合**：`{"catchphrase": chain1} | chain2` が最大のポイントです。これは「まずchain1を実行し、その結果を `catchphrase` という名前の変数としてchain2に渡せ！」という見事なバケツリレーの指示になります。
