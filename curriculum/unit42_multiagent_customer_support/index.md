...
下図は、 **Triage（受付）** から Billing / Tech / Sales へ振り分けるマルチエージェントの流れです（図は一般的な部署分けの例で、本文の配送・決済エージェント構成と役割名は異なりますが構造は同じです）。
...
まずは実装例の2エージェント構成を動かしてください。次に、 **「キャンセル手数料、配送DB、ポイント還元規約を分離して調査するマルチエージェント協調システム」** を設計し、最後に権限、引数検証、根拠、エスカレーション条件を設計メモにまとめます。ハルシネーションゼロは前提にしません。
