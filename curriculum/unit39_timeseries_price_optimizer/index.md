...
<img src="../../assets/units/unit39_timeseries_price_optimizer/images/diagram-concept.svg" alt="図解：時系列データの機械学習" class="unit-diagram" />
...
<img src="../../assets/units/unit39_timeseries_price_optimizer/images/diagram-workflow.svg" alt="図解：ダイナミックプライシングの流れ" class="unit-diagram" />
...
実務におけるシニアデータサイエンティストとして、 **「時系列データリークを徹底的に回避しつつ、販促イベントや競合価格の影響を学習させ、最大収益をもたらす最適価格決定アルゴリズム」** を自分で設計し、実装してください。

**【課題の要件】**
以下の「生データ（祝日フラグ、競合の動的な価格設定、過去の売上ログ）」を初期化コードとして使用し、ここから **「予測リークを100%排除した評価パイプライン」** を作成の上、最適な価格決定戦略を導き出してください。
...
| :--- | :--- | :--- | :--- |
| **異常な価格設定の抑止力** | **非常に弱い** 。モデルの予測外の極端な超高価格（例: 5,000円）において、XGBoostが「需要は少し減るだけ」と誤判定し、売上が無限大になると計算してしまうリスクがある。 | **非常に強い** 。過去の実験的価格の最小値〜最大値（例: 500円〜1,500円）の範囲に探索境界を厳格に限定するため、現実離れした暴走価格を100%防止できる。 | 機械学習の「外挿（学習データ外の予測）」の弱さを知ったうえで、 **探索境界（Bounds）を課すのが必須要件** となります。 |
| **競合価格の追従性** | 考慮しない。自社の価格変動だけで売上を予測するため、競合が超値下げした時に顧客を全滅させられるリスクがある。 | 考慮する。特徴量に「競合との価格差（Our Price - Competitor Price）」などの相対的な特徴量を明示的に設計することで、競合の出方に応じた動的追従が可能。 | 実務では競合との相対価格比こそが需要を動かす最強の変数であるため、この設計判断が必須です。 |
...
* **最終適用判断（Decision）** :
  ...
  * **意思決定の根拠** :
    1. **リークを完全に回避したモデルの信頼性** : 時系列分割（TimeSeriesSplit）による厳密な検証を経て、予測MAEが「実データに対して極めて小さい誤差範囲」に収まっていることを科学的に証明しています。
    ...
    3. **最適化境界（Bounds）による安全運行** : モデルの数理的暴走（外挿予測）を抑えるために、過去の実験履歴である `600円〜1300円` の境界で探索を行っているため、システムが突然「1円で売れば完売して儲かる」や「1万円で1個売ればいい」といった破壊的なエラー価格を設定するリスクを事前に100%遮断しています。
</details>
