...
class SimpleMLP(nn.Module):
    def __init__(self):
        ...
        self.sigmoid = nn.Sigmoid()                            # 隠れ層の活性化関数

    def forward(self, x):
        # 組み立て方（データがどう流れるか）
        x = self.hidden(x)
        x = self.sigmoid(x)
        return self.output(x)  # 最終出力は確率化せず logits のまま返す
...
```python
# 3. 損失関数と最適化手法の定義
criterion = nn.BCEWithLogitsLoss() # logitsを受け取る数値的に安定した2値分類用の損失
...
- 出力層：1 （出力は確率化せず logits のまま返す）
- 損失関数は `nn.BCEWithLogitsLoss()`（logitsを直接受け取る2値分類用）を使ってみてください。
...
今回の変更点には、それぞれ意味があります。まず隠れ層の活性化関数を Sigmoid から **ReLU に変えたのは「非線形性」を効率よく導入するため** です。もし活性化関数を挟まずに `nn.Linear` を重ねるだけだと、何層積んでも結局は1つの線形変換と同じになってしまいます。ReLU は「0以下なら0、正ならそのまま通す」ため、深いネットワークでも学習しやすい活性化関数です。出力層は Sigmoid を通さず logits のまま返し、`BCEWithLogitsLoss` に渡します。この損失関数は Sigmoid と BCELoss を内部で組み合わせて数値的に安定して計算するため、モデル側で Sigmoid を重ねません。回帰では `MSELoss`、二値分類では `BCEWithLogitsLoss` を使い分けます。
