...
> [!TIP]
> ここで登場する **BatchNorm（バッチ正規化）** は、[Unit 13（過学習防止策）](../unit13_regularization/index.md) で学んだ正則化手法の一つです。各層の出力を正規化して学習を安定・高速化する手法で、この演習ではCNNの畳み込み層に `nn.BatchNorm2d()` として実際に組み込みます。
...
ここでは、PyTorchと `torchvision` のユーティリティを使い、ランダムに生成した画像データをカスタムデータセットとして読み込み、`ResNet18` を転移学習させて訓練曲線と精度曲線をMatplotlibでプロットする完全なパイプラインを実装します。
...
| :--- | :--- | :--- | :--- |
| **過学習リスク** | **極めて低い**。パラメータ数が最小限に抑えられているため、単純な境界線しか引けず、過学習しにくい。 | **極めて高い**。モデルサイズに対してデータが120枚と少なすぎるため、過学習対策（強めのDropout）が絶対に必要。 |
| **学習効率** | ゼロから重みを学習するため、このデータサイズでは十分な特徴量を抽出する前に学習が頭打ちになりやすい。 | **圧倒的に高い**。すでに「エッジ」「形状」などの汎用的な特徴を事前学習で獲得しているため、極小データでも即座に適応。 |
| **入力サイズ制限** | 64x64ピクセルに合わせたカーネル設計ができるため、無駄なアップスケーリングが不要で高速。 | ResNet等の標準入力（224x224）にリサイズするか、64x64のまま通すか。今回は64x64のままでも動くが特徴が潰れやすい。 |
| **運用効率（推論コスト）** | **最強に良い**。モデルサイズが数キロバイト〜数十メガバイトで済み、超高速かつCPUでも快適に動作する。 | **悪い**。ただの2クラス分類に対してモデルが巨大すぎ、推論に必要なメモリやGPUコストが大きくなる。 |
...
このタスクでは、検証データ（ランダムに作られたわずか30枚）に対する結果を比較すると、**アプローチB（ResNet18転移学習）の方が、少ないエポック数で圧倒的に早く、かつ高い検証精度を達成する**ことが実証されます。
