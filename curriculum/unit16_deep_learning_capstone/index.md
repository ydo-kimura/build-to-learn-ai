...
| :--- | :--- | :--- | :--- |
| **過学習リスク** | **極めて低い** 。パラメータ数が最小限に抑えられているため、単純な境界線しか引けず、過学習しにくい。 | **極めて高い** 。モデルサイズに対してデータが120枚と少なすぎるため、過学習対策（強めのDropout）が絶対に必要。 | データが極小の今回は、過学習対策の設計が精度を左右する最大の要因になる。 |
| **学習効率** | ゼロから重みを学習するため、このデータサイズでは十分な特徴量を抽出する前に学習が頭打ちになりやすい。 | **圧倒的に高い** 。すでに「エッジ」「形状」などの汎用的な特徴を事前学習で獲得しているため、極小データでも即座に適応。 | 極小データでは「ゼロから学ぶ」より「借りてきて微調整する」方が圧倒的に有利。 |
...
| **運用効率（推論コスト）** | **最強に良い** 。モデルサイズが数キロバイト〜数十メガバイトで済み、超高速かつCPUでも快適に動作する。 | **悪い** 。ただの2クラス分類に対してモデルが巨大すぎ、推論に必要なメモリやGPUコストが大きくなる。 | 精度が同等なら、運用コストの安いモデルを選ぶのがアーキテクトの鉄則。 |
