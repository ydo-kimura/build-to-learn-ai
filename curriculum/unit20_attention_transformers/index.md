...
この問題を大きく改善し、現代のAI（ChatGPTなど）の基盤になったのが **Attention（注意機構）** と、それを使った **Transformer** というアーキテクチャです。Transformerにも計算量、コンテキスト長、位置情報、推論速度などの制約があります。
...
4. 位置情報を加えた `query`, `key`, `value` をAttention層に入力し、出力結果 (`attn_output`) と重みを表示してください。
...
- 位置情報は `query = query + positional_encoding` のように加算します。
...
key = key + positional_encoding
value = value + positional_encoding
