...
graph TD
    ...
    A["LLM<br>Generate text"]:::main

    A -->|Bring in external knowledge| B["RAG<br>Search external data"]:::sub

    A -->|Tool use & autonomous action loop| C["AI Agent<br>Plan and use tools"]:::highlight

    C -->|Engine of the AI agent| CL["Agent Loop<br>Think → Act → Observe"]:::loop

    CL -->|Hands and feet| TC["Tool Calling<br>Run external tools"]:::loop

    TC -->|Standard connector for tools| MCP["MCP<br>Connect external tools"]:::loop

    B -->|Add autonomy, branching & retry| D["Agentic RAG<br>Search and retry"]:::highlight
    C -->|Integrate RAG as a tool| D

    C -->|Coordination & role division| MA["Multi-Agent<br>Agents cooperate"]:::highlight
```
