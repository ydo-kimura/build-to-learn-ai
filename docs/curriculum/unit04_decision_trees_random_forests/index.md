...
決定木はデータを分析し、評価指標上よく分けられる質問（分割条件）を探して、ツリー（木）を作っていきます。
...
| 決定木のメリット                                     | 決定木のデメリット                                                   |
| :--------------------------------------------------- | :------------------------------------------------------------------- |
| なぜその予測になったか、人間が図を追いやすい         | 過去のデータを丸暗記しすぎて、応用が利かなくなる（過学習しやすい）   |
...
下図では、3本の木がそれぞれクラス A または B に投票し、 **多数決（Majority vote）** で最終予測 A が選ばれる流れを示しています。実装では `n_estimators=100` のように木の本数を指定し、同じ仕組みを大規模に行います。

<img src="../../assets/units/unit04_decision_trees_random_forests/images/diagram-workflow.svg" alt="図解：ランダムフォレストの多数決（Tree1→A, Tree2→A, Tree3→B → 最終A）" class="unit-diagram" />
...
**【コードの解説】**
`RandomForestClassifier` がランダムフォレストのクラスです。先ほどの `DecisionTree` と使い方は全く同じですが、内部で100本の木を作って多数決をとってくれています。結果を比べると、多くの場合ランダムフォレストの方が正解率が高くなる傾向があり、より安定した予測ができることが分かります。

また、決定木の過学習を体感するには、`max_depth`（木の深さ）を変えて訓練データとテストデータの正解率の差を観察してみるのがおすすめです。

```python
# max_depth の有無で過学習を観察してみましょう
deep_tree = DecisionTreeClassifier(max_depth=None, random_state=42)  # 深さ制限なし（過学習しやすい）
shallow_tree = DecisionTreeClassifier(max_depth=3, random_state=42)  # 深さを3に制限

for name, tree in [("制限なし", deep_tree), ("max_depth=3", shallow_tree)]:
    tree.fit(X_train, y_train)
    train_acc = accuracy_score(y_train, tree.predict(X_train))
    test_acc = accuracy_score(y_test, tree.predict(X_test))
    print(f"{name}: 訓練 {train_acc:.3f} / テスト {test_acc:.3f}")
```

実行すると、深さ制限なしの木は訓練データでほぼ100%の正解率になる一方、テストデータでは伸び悩むことが確認できます。この「訓練とテストの差」が過学習のサインです。
...
ランダムフォレストは、表形式データで試しやすい代表的なベースラインの一つです。どんなデータでも最適とは限らないため、単純モデルや他のアンサンブルと比較します。
</details>
