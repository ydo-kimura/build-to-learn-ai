...
<p class="unit-hero">
  <img src="../../assets/units/unit02_logistic_regression/images/hero.png" alt="ヒーロー画像：スコアを0〜1の確率に変換するシグモイド関数" />
</p>
...
下図は、線形回帰の直線では予測値が0〜1の範囲を飛び出してしまうのに対し、シグモイド関数なら必ず0〜1の確率に収まる様子を比較したものです。

<img src="../../assets/units/unit02_logistic_regression/images/diagram-sigmoid.svg" alt="図解：線形回帰の不正な出力 vs シグモイドの0〜1確率" class="unit-diagram" />
...
下図は、予測と実際の組み合わせを4マスに整理した混同行列と、そこから適合率・再現率を計算する式を表したものです。

<img src="../../assets/units/unit02_logistic_regression/images/diagram-confusion-matrix.svg" alt="図解：混同行列と適合率・再現率の計算式" class="unit-diagram" />
...
なお、上の表の適合率・再現率は、上図の混同行列の各マス（TP/FP/FN/TN）に対応しており、そこから計算されます。

> [!TIP]
> **不均衡データと ROC-AUC**
> 「オオカミ少年」の例のように、クラスの比率が大きく偏ったデータを **不均衡データ（Imbalanced Data）** と呼びます。不均衡データでは正解率が当てにならないため、適合率・再現率・F1スコアで評価するのが鉄則です。また、scikit-learn では `LogisticRegression(class_weight="balanced")` と指定するだけで、少数クラスの間違いに大きなペナルティを課して学習させることができます。
> もう一つ実務でよく使われるのが **ROC-AUC** という指標です。判定のしきい値（何%以上を陽性とみなすか）を動かしたときの性能を総合した0〜1のスコアで、`sklearn.metrics.roc_auc_score(y_test, model.predict_proba(X_test)[:, 1])` の1行で計算できます。1に近いほど「陽性と陰性をきれいに順位付けできるモデル」です。
...
**【コードの解説】**
`LogisticRegression` モデルを作成し、`.fit()` で学習させます。（`max_iter=10000` は「諦めずに1万回まで計算してね」という試行回数の上限指定です。このデータは特徴量ごとの数値のスケールがバラバラなため計算の収束に時間がかかりますが、Unit 3 で学ぶ「標準化」を行うと、ずっと少ない回数で収束するようになります）。
...
**【解答コードの解説】**
ロジスティック回帰は本来「2択（バイナリ）」の分類が得意ですが、scikit-learnのロジスティック回帰は自動的に「3つ以上のカテゴリ（多クラス分類）」にも対応してくれます。2クラス分類で使ったシグモイド関数を多クラスに拡張したものが **ソフトマックス関数** で、「農家A: 70%、農家B: 20%、農家C: 10%」のように全カテゴリの確率を同時に計算し、一番高いものを答えとして返します。
</details>
