...
この Unit では、Notebook で Python を実行し、NumPy 配列、pandas の表、データセットの `X` と `y`、学習用・テスト用への分割を扱います。初めての項目や説明できない項目があれば、[Unit 0 の事前準備の案内](../unit00_roadmap/index.md)から、各 Prep のスキップ判定を確認してください。

- Notebook のセルや実行順に不安がある: [Prep 1](../prep01_notebook_colab/index.md)
- Python の変数、関数、インポートに不安がある: [Prep 2](../prep02_python_basics/index.md)
- `ndarray` の形、行、列に不安がある: [Prep 3](../prep03_numpy/index.md)
- 表の列名、`DataFrame`、`Series`、`X` と `y` に不安がある: [Prep 4](../prep04_pandas/index.md)
- データの観察、尺度調整、分割後の前処理に不安がある: [Prep 5](../prep05_data_preprocessing/index.md)

各 Prep は、実際にできるかと説明できるかで進むかを決めます。答えを見てわかったつもりになるのではなく、スキップ判定と実践で手を動かして確認してください。
