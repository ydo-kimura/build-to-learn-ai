...
<img src="../../assets/units/unit29_ai_agent_implementation/images/diagram-concept.svg" alt="図解：ReAct ループ（推論と行動の繰り返し）" class="unit-diagram" />
...
LLM自身がコードを実行するのではなく、 **「プログラム側でツールを実行するための指示書（JSON）」をLLMが出力し、実行自体はホストプログラム（Pythonなど）側が安全に行う** という協調設計になっています（ツールの実行はあなたのアプリのプロセス内で行われます）。

なお、元論文の ReAct は「Thought / Action / Observation をプレーンテキストとして書かせ、それをパースする」形式でしたが、本ユニットの実装は、それを **構造化された関数呼び出し（Tool Calling）でループを回す現代的な方式** に置き換えたものです。つまり、思考と行動を交互に繰り返すという ReAct の思想はそのままに、壊れやすいテキストパースを OpenAI Tool Calling の堅牢な JSON スキーマで実現しています。

下図は、ツール定義から実行までの **スクラッチエージェント** の構成です。

<img src="../../assets/units/unit29_ai_agent_implementation/images/diagram-workflow.svg" alt="図解：スクラッチで作るエージェントの構成" class="unit-diagram" />
...
ここでは、OpenAIのTool Callingを用い、エージェントが計算機ツールと在庫検索ツールを選択してユーザーの質問に答える、最小のReAct実行ループをスクラッチ実装します。実務では最大反復回数、タイムアウト、権限、引数検証、失敗時のエスカレーションを追加します。

> **Colab セットアップ:** 現行の Colab には OpenAI Python SDK が含まれているため追加インストールは不要です。環境変数に `OPENAI_API_KEY` を設定してください。
...
以下は、自律返品審査ReActエージェントをOpenAI APIを用いてスクラッチ実装した解答例です。
