...
RNNやLSTMは「記憶をバケツリレーのように渡していく」仕組みでした。しかし、長い文章になると、離れた位置の情報を保持しにくい問題がありました。
この問題を大きく改善し、現代のAI（ChatGPTなど）の基盤になったのが **Attention（注意機構）** と、それを使った **Transformer** というアーキテクチャです。Transformerにも制約があり、特にSelf-Attentionの計算量は系列長の二乗に比例するため、長文では計算量やコンテキスト長への工夫が必要です。位置情報の表現や生成時の推論速度も設計上の論点になります。
...
| :-------------------------- | :--- | :------- | :----- | :---- | :--- | :----- | :------ | :--- | :--- | :--- | :---- |
| it が参照する重み（概念例） | 0.02 | **0.35** | 0.01   | 0.02  | 0.01 | 0.03   | 0.04    | 0.08 | 0.10 | 0.05 | 0.29  |

※この行は説明用の仮想的な重みです。実際の重みは学習済みパラメータや入力によって変わります。

この英文の「it（それ）」は何を指しているでしょうか？人間なら、「tired（疲れている）」のは「animal（動物）」だと考えやすいでしょう（streetが疲れることはないからです）。
Self-Attentionは、学習済みのQ/K/V変換を通して、文脈上重要なトークンから情報を集めます。図や手作りのベクトルだけで意味を理解するわけではなく、実際の関係性は学習データと重み、プロンプト、マスクなどの影響を受けます。

下図は、 **Query × Key** で得た重みを使い、各トークンが他トークンを参照する Attention です。

図はQ/K/Vの関係を中心に示した概念図です。実装では、語順を区別するための位置情報を別途入力に加えます。
...
RNNのように入力系列を一つずつ処理する代わりに、訓練時のSelf-Attentionでは系列内の関係をまとめて計算できます。これによりGPUで並列化しやすくなり、巨大なモデルの学習を支えました。ただし、生成時のデコーダーは通常、次のトークンを一つずつ生成します。
...
実装上は、入力の埋め込みベクトルにこの位置情報を加算してからAttention層へ渡します。後述の最小コードでは計算の核心を見せるため省略しますが、実用的なTransformerの実装では省略できません。
...
- `Q` と `K` を掛け算（内積）し、キーの次元数 `d_k` の平方根 `√d_k` で割って、「どのくらい関連しているか（Attention Score）」を出します。これにより、次元数が大きいときもSoftmaxが極端に飽和しにくくなります。
...
今度は、実際の PyTorch に用意されている `nn.MultiheadAttention` という完成済みのレイヤーを使い、位置エンコーディングを加えてAttentionの計算を行ってみましょう。

**【課題の要件】**

1. 以下の入力埋め込みデータ（ランダムな値）を使用してください。この課題では形状とAPIの使い方を確認するため、重みが特定の単語関係を表すとは限りません。
2. 正弦波または学習可能な位置エンコーディングを実装してください。
3. `nn.MultiheadAttention` を使って、Attention層（`embed_dim=8`, `num_heads=2`）を作成してください。
4. 位置情報を加えた入力埋め込みを `query`, `key`, `value` としてAttention層に入力し、出力結果 (`attn_output`) と重みを表示してください。
...
- 位置情報は、Attention層に渡す前の入力埋め込みに加算します。`query`, `key`, `value` に同じ位置情報を加える実装は、今回の簡略例では行いません。
- 使い方: `attn_output, attn_weights = attention_layer(embedded, embedded, embedded)`
...
# Attention層に渡す前の入力埋め込みを作成
embedded = torch.rand(sequence_length, batch_size, embed_dim)

# 2. 正弦波による位置エンコーディングを作成
...
print(attn_output.shape) # 形が元の入力埋め込みと同じ (5, 1, 8) になることを確認
print(attn_output)
