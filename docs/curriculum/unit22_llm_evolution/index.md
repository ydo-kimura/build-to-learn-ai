...
graph TD
    ...
    A -->|外部知識の持ち込み| B["RAG<br>検索して回答"]:::sub

    A -->|道具の使用・自律的な行動ループ| C["AI Agent<br>計画して実行"]:::highlight

    C -->|AIエージェントのエンジン| CL["Agent Loop<br>思考→行動→観察"]:::loop

    CL -->|手足となる技術| TC["Tool Calling<br>外部ツールを実行"]:::loop

    TC -->|ツール接続の標準規格| MCP["MCP<br>ツール接続規格"]:::loop

    B -->|自律性と分岐・リトライの追加| D["Agentic RAG<br>検索と再試行"]:::highlight
    C -->|RAGをツールとして統合| D

    C -->|協調と役割分担| MA["Multi-Agent<br>Agentが協調"]:::highlight
```
